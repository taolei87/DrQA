...
extern "C" {
    ...
    __global__ void knn_bi_fwd(const float * __restrict__ u, const float * __restrict__ x,
                            const float * __restrict__ bias, const float * __restrict__ init,
                            const float * __restrict__ mask_h,
                            const int len, const int batch, const int d, const int k,
                            float * __restrict__ h, float * __restrict__ c,
    ...
    __global__ void knn_bi_bwd(const float * __restrict__ u, const float * __restrict__ x,
                            const float * __restrict__ bias, const float * __restrict__ init,
                            const float * __restrict__ mask_h, const float * __restrict__ c,
                            const float * __restrict__ grad_h, const float * __restrict__ grad_last,
                            const int len, const int batch, const int d, const int k,
...
KNN_BiFWD_FUNC = KNN_MOD.get_function('knn_bi_fwd')
KNN_BiBWD_FUNC = KNN_MOD.get_function('knn_bi_bwd')
...
class KNN_Compute(Function):
    ...
    def forward(self, u, x, bias, init=None, mask_h=None):
        ...
        FUNC = KNN_FWD_FUNC if not self.bidirectional else KNN_BiFWD_FUNC
        FUNC(args=[
    ...
    def backward(self, grad_h, grad_last):
        ...
        FUNC = KNN_BWD_FUNC if not self.bidirectional else KNN_BiBWD_FUNC
        FUNC(args=[
            u.contiguous().data_ptr(),
            x.contiguous().data_ptr() if k_ == 3 else 0,
            bias.data_ptr(),
...
class FastKNNCell(nn.Module):
    ...
    def forward(self, input, c0=None):
        ...
        if self.training and (self.dropout>0):
            ...
            h, c = KNN_Compute(self.use_tanh, n_out, self.bidirectional)(u, input, self.bias, c0, mask_h)
        else:
            h, c = KNN_Compute(self.use_tanh, n_out, self.bidirectional)(u, input, self.bias, c0)
...
class FastKNN(nn.Module):
    def __init__(self, n_in, n_out, depth, dropout=0, rnn_dropout=0,
                use_tanh=0, bidirectional=False):
        ...
        self.bidirectional = bidirectional
        self.out_size = n_out*2 if bidirectional else n_out

        for i in range(depth):
            l = FastKNNCell(
                n_in = n_in if i==0 else self.out_size,
                n_out = n_out,
    ...
    def add_bias(self, bias_val=0):
        ...
        for l in self.rnn_lst:
            if self.bidirectional:
                l.bias.data[n_out*2:].add_(bias_val)
            else:
                l.bias.data[n_out:].add_(bias_val)
